...
import { z } from 'zod';
import { AUTHORING_GUIDE, schemaText, validateScene, saveScene } from './tools';
...
export function createServer(): McpServer {
  ...
  server.registerTool(
    'get_scene_schema',
    {
      title: '取得 SceneTimeline 的 JSON Schema 與撰寫指南',
      description:
        '回傳 FrameForge SceneTimeline 的完整 JSON Schema 與簡短撰寫指南。產生場景前先呼叫此工具了解格式。',
      inputSchema: {},
    },
    async () => ({
      content: [
  ...
  server.registerTool(
    'validate_scene',
    {
      title: '驗證 SceneTimeline',
      description:
        '用 scene-schema 驗證一份 timeline（物件或 JSON 字串），回傳是否通過與可據以修正的錯誤清單。' +
        '產生場景後務必呼叫；未通過就依錯誤修正再驗，直到通過。',
  ...
  server.registerTool(
    'save_scene',
    {
      title: '驗證並存檔 SceneTimeline',
      description:
        '先驗證，通過才把 timeline 寫成 JSON 檔（給 Studio「載入場景」用）；未通過不寫、回傳錯誤。',
